...
func intersections(_ p1: [[Int]], _ p2: [[Int]]) -> Set<[Int]> {
    let p1 = Set(p1)
    let p2 = Set(p2)
...
func closestIntersection(_ intersects: Set<[Int]>) -> Int {
    let values = intersects.map { abs($0[0]) + abs($0[1]) }
    let sorted = values.sorted()

    return sorted[1]
}

func fewestStepIntersection(_ intersects: Set<[Int]>, _ p1: [[Int]], _ p2: [[Int]]) -> Int {
    var steps = 0

    intersects.forEach { i in
        let s1 = p1.firstIndex(of: i) ?? 0
        let s2 = p2.firstIndex(of: i) ?? 0
        let sum = s1 + s2

        if steps == 0 || sum < steps && sum != 0 {
            steps = sum
        }
    }
...
let p1 = points(String(wires[0]))
let p2 = points(String(wires[1]))
let intersects = intersections(p1, p2)

debugPrint(closestIntersection(intersects))
debugPrint(fewestStepIntersection(intersects, p1, p2))
