...
opts = %w[one two three four five six seven eight nine].map.with_index { [_1, (_2 + 1).to_s] }.to_h
res = ->{ [_1[0] + _1[-1]].join.to_i }

p input.sum { _1.scan(/\d/).then(&res) }
p input.sum { _1.scan(/(?=(\d|#{Regexp.union(opts.keys)}))/).flatten.map { |v| opts[v] || v }.then(&res) }

# "five8eight8zrrkrvbjzcqbtwoeightwogvd" = fu
